...
def sky_model(args, data, field, probabilities):
   ...
   plt.savefig('./%s/%s_sky_ellipse.pdf'%(args.name, args.name), bbox_inches='tight')
   plt.close('all')
   ...
   s = sectors_photometry(img, f.eps, f.theta, f.xpeak, f.ypeak, minlevel=minlevel, mask=mask, plot=0)
   ...
   plt.savefig('./%s/%s_mpg_profiles.pdf'%(args.name, args.name), bbox_inches='tight')
   plt.close('all')
...
def CalculateProbabilities(args, data, field, systematics = None, probs_filename = 'None', ellipse_filename = 'None'):
   ...
   #for idx in data.index:
         #test1 = idx
         #print(stats.norm.pdf(data.loc[data.index != idx, "gmag"], data.loc[data.index == idx, "gmag"], data.loc[data.index != idx, "gmag_error"]))
         #print(stats.norm.pdf(data.loc[data.index != idx, "bp_rp"], data.loc[data.index == idx, "bp_rp"], data.loc[data.index != idx, "bp_rp_error"]))
         #print(np.sum(stats.norm.pdf(data.loc[data.index != idx, "gmag"], data.loc[data.index == idx, "gmag"], data.loc[data.index != idx, "gmag_error"]) * \
                                    #stats.norm.pdf(data.loc[data.index != idx, "bp_rp"], data.loc[data.index == idx, "bp_rp"], data.loc[data.index != idx, "bp_rp_error"])))
...
def manual_select_from_cmd(args, data):
   ...
   nstars = ax1.annotate('%i stars'%members.sum(), xy=(np.nanmin(data_color)+0.1, np.nanmin(data_mag)+0.3), xycoords='data', color = 'k')
...
def plot_final_selection(args, data, plot_prob = True):
   ...
   nstars = ax1.annotate('%i stars'%members.sum(), xy=(np.nanmin(data_color)+0.1, np.nanmin(data_mag)+0.7), xycoords='data', color = 'k')
   ...
   ax2.plot(args.pmra, args.pmdec, 'rx', ms = 3, zorder = 2)
   ax5.plot(args.pmra, args.pmdec, 'rx', ms = 3, zorder = 2)
...
def main(argv):  
   ...
   parser.add_argument('--membership_prob', type = float, default = 0.7, help = 'Performs a first selection using maximum likelihood.')
   ...
   quality_cols = 'visibility_periods_used, astrometric_excess_noise_sig, astrometric_params_solved, astrometric_n_good_obs_al, astrometric_chi2_al, phot_bp_rp_excess_factor, ruwe, (phot_bp_n_blended_transits+phot_rp_n_blended_transits) *1.0 / (phot_bp_n_obs + phot_rp_n_obs) AS beta, ipd_gof_harmonic_amplitude, phot_bp_n_contaminated_transits, phot_rp_n_contaminated_transits'
   ...
   clean_label = clean_astrometry(table['ruwe'], table['ipd_gof_harmonic_amplitude'], table['visibility_periods_used'], table['astrometric_excess_noise_sig'], table['astrometric_params_solved'], use_5p = True)
      ...
      if args.do_first_pass:
         ...
         try: 
            ...
            probabilities = CalculateProbabilities(args, data.copy(), field.copy(),
                                                   systematics = systematics, probs_filename = "./%s/%s_probabilities.csv"%(args.name, args.name), ellipse_filename="./%s/%s_ellipse.csv"%(args.name, args.name))

            data.loc[:, 'membership_prob'] = probabilities.loc[:, 'membership_prob'].values

         field.loc[:, 'membership_prob'] = 0.0
         if args.membership_prob is not None:
            data.loc[:, 'member_1_pass'] = data.loc[:, 'membership_prob'] >= args.membership_prob
         else:
   ...
   try:
      ...
      data['radial_velocity'] = args.radial_velocity
      data['distance'] = args.distance

   try:
      data_filt, data_voronoi, voronoi_cells, index_sel, index_rej = voronoi_stats_parallel(data[data.member == True], ['x', 'y', 'pmra', 'pmdec'], [['x','y']], ['voronoi_cell'], [int(np.sqrt(len(data[data.member == True])))], sigma = 3)
      data.loc[data.member == True, 'member'] = data.loc[data.member == True].index.isin(index_sel[0])
      data['voronoi_cell'] = voronoi_cells[0]
   except:
      pass
   
   stats = statistics(data.loc[data.member, ['ra', 'dec', 'pmra', 'pmdec', 'ra_error', 'dec_error', 'pmra_error', 'pmdec_error', 'radial_velocity']])
   ...
   data.to_csv("./%s/%s_%s.csv"%(args.name, args.name, args.com_option), index = False)

   stats['majoraxis'] = args.majoraxis
   stats['eps'] = args.eps
   stats['pa'] = args.pa
   stats['nstars'] = data.member.sum()
   stats.to_frame().T.to_csv("./%s/%s_%s_resumen.csv"%(args.name, args.name, args.com_option), index = False)

   try:
      field.to_csv("./%s/%s_field.csv"%(args.name, args.name), index = False)
      probabilities.to_csv("./%s/%s_probabilities.csv"%(args.name, args.name, args.com_option), index = False)
   except:
      pass
