...
def sky_model(args, data, field, probabilities, fit_pge = True, use_center = True):
...
def CalculateProbabilities(args, data, field, systematics = None, probs_filename = 'None', ellipse_filename = 'None'):
   ...
   if args.use_position_prob:
      ...
      try:
         ellipse, probabilities = sky_model(args, data, field, probabilities, fit_pge = True, use_center = True)
...
def main(argv):  
   ...
   clean_label = clean_astrometry(table['ruwe'], table['ipd_gof_harmonic_amplitude'], table['visibility_periods_used'], table['astrometric_excess_noise_sig'], table['astrometric_params_solved'], use_5p = False)
   ...
   if args.use_members:
      ...
      prepare_for_clustering = True

      if args.prepare_for_clustering:
         pmra_select = np.abs(data.pmra - args.pmra)-data.pmra_error  < 0.5
         pmdec_select = np.abs(data.pmdec - args.pmdec)-data.pmdec_error  < 0.5
   ...
   try:
      data_filt, data_voronoi, voronoi_cells, index_sel, index_rej = voronoi_stats_parallel(data[data.member == True], ['x', 'y', 'pmra', 'pmdec'], [['x','y']], ['voronoi_cell'], [min(1, int(np.sqrt(len(data[data.member == True]))))], sigma = 3)
   ...
   ellipse = sky_model(args, data, field, probabilities, fit_pge = False, use_center = False)
