...
def clean_astrometry(phot_g_mean_mag, bp_rp, astrometric_chi2_al, astrometric_n_good_obs_al, uwe = None, norm_uwe = True):
   """
...
def pre_clean_data(phot_g_mean_mag, bp_rp, astrometric_chi2_al, astrometric_n_good_obs_al, phot_bp_rp_excess_factor, uwe = None, norm_uwe = True):
   """
   ...
   labels_astrometric, uwe = clean_astrometry(phot_g_mean_mag, bp_rp, astrometric_chi2_al, astrometric_n_good_obs_al, uwe = uwe, norm_uwe = norm_uwe)
...
def gaia_query(Gaia, query, min_gmag, max_gmag, norm_uwe, test_mode, save_individual_queries, load_existing, name, n, n_total):
   ...
   if not test_mode:
      ...
      if os.path.isfile(individual_query_filename) and load_existing:
         ...
         result['clean_label'], uwe = pre_clean_data(result['gmag'], result['bpmag'] - result['rpmag'], result['astrometric_chi2_al'], result['astrometric_n_good_obs_al'], result['phot_bp_rp_excess_factor'], uwe = uwe, norm_uwe = norm_uwe)
...
def main(argv):  
   ...
   parser.add_argument('--source_table', type = str, default = 'gaiadr2.gaia_source', help='Gaia source table. Default is gaiadr2.gaia_source.')
   ...
   parser.add_argument('--remove_quality_cols', type = str2bool, default = True, help='If True, the code will remove all quality columns from the final table, except "clean_label".')
   ...
   astrometric_cols = 'ra, ra_error, dec, dec_error, parallax, parallax_error, pmra, pmra_error, pmdec, pmdec_error, radial_velocity, radial_velocity_error, ra_dec_corr, ra_parallax_corr, ra_pmra_corr, ra_pmdec_corr, dec_parallax_corr, dec_pmra_corr, dec_pmdec_corr, parallax_pmra_corr, parallax_pmdec_corr, pmra_pmdec_corr'
   
   photometric_cols = 'phot_g_mean_mag AS gmag, (1.09*phot_g_mean_flux_error/phot_g_mean_flux) AS gmag_error, phot_bp_mean_mag AS bpmag, (1.09*phot_bp_mean_flux_error/phot_bp_mean_flux) AS bpmag_error, phot_rp_mean_mag AS rpmag, (1.09*phot_rp_mean_flux_error/phot_rp_mean_flux) AS rpmag_error, bp_rp'
