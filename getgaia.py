...
def plot_final_selection(args, data, ellipse = None, plot_prob = True):
   ...
   ax2.plot(args.pmra_new, args.pmdec_new, 'rx', ms = 3, zorder = 2)
   ax5.plot(args.pmra_new, args.pmdec_new, 'rx', ms = 3, zorder = 2)
...
#def get_real_error(table):
   #"""
   #This routine calculates the excess of error that should be added to the listed Gaia errors. The lines are based on Fabricius 2020. Figure 19.
   #"""

   #p5 = table.astrometric_params_solved == 31                                        # 5p parameters solved Brown et al. (2020)
   #p5_error = -0.025 * table.loc[p5, 'gmag'] + 1.55
   #p6_error = -0.033 * table.loc[~p5, 'gmag'] + 1.86
   
   #table.loc[:, ['parallax_error_old', 'pmra_error_old', 'pmdec_error_old']] = table.loc[:, ['parallax_error', 'pmra_error', 'pmdec_error']].values
   
   #table.loc[p5, ['parallax_error', 'pmra_error', 'pmdec_error']] = np.sqrt(table.loc[p5, ['parallax_error', 'pmra_error', 'pmdec_error']].values**2 * np.expand_dims(p5_error.values, axis =1 )**2)
   #table.loc[~p5, ['parallax_error', 'pmra_error', 'pmdec_error']] = np.sqrt(table.loc[~p5, ['parallax_error', 'pmra_error', 'pmdec_error']].values**2 * np.expand_dims(p6_error.values, axis =1 )**2)
...
def main(argv):  
   ...
   parser.add_argument('--use_position_prob', type = str2bool, default = True, help = 'Calculate probabilities of a star being a member using Multi Gaussian Expansion.')
